            ...
            for(int search = 0 ;search < k-1;search++){
                if(magic[search]->rabin == magic[search+1]->rabin){
                    ...
                    while(search+1<k && magic[search]->rabin == magic[search+1]->rabin ){
                        if (magic[search]->initialrabin == magic[search+1]->initialrabin){
                            havesame = true;
                            countsame++;
                        }
                        printf("\n%d %d\n",magic[search]->origindex,magic[search+1]->origindex);
                        countsimilar++;
                        search++;
                    }
                    countsame++;
                    countsimilar++;
                    if(havesame == true){
                        printf("there are %d same and %d similiar in these set\n",countsame,countsimilar);
                        ans += countC(countsimilar)-countC(countsame);
                        printf("we add %d\n",countC(countsimilar)-countC(countsame));
                    }
                    else{
                        printf("there are %d same and %d similiar in these set\n",countsame,countsimilar);
